...
# -----------------------------------------------------------------------------
import asyncio
from typing import Optional, Dict, Text, NoReturn, List
...
class SlackAppTKParser(ArgumentParser):
    ...
    @staticmethod
    async def send_version(rqst, versiontext) -> None:
        await Response(rqst).send_response(text=versiontext)

    @staticmethod
    def form_help_on_error(rqst, errmsg, helptext) -> List:
        atts = list()
...
class SlashCommandCLI(object):
    ...
    async def run(self, rqst, event=None):
        ...
        return await handler(rqst, ns_args)
...
class _TkHelpAction(_HelpAction):
    ...
    def __call__(self, parser: SlackAppTKParser, namespace, values, option_string=None):
        ...
        asyncio.create_task(parser.send_help(rqst))
        raise SlackAppTKParserExit()
...
class _TkVersionAction(_VersionAction):
    ...
    def __call__(self, parser: SlackAppTKParser, namespace, values, option_string=None):
        ...
        asyncio.create_task(parser.send_version(rqst, formatter.format_help()))
        raise SlackAppTKParserExit()
